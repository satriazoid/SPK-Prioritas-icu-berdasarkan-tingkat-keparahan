...
<style>
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  @keyframes pulse {
    0%,100% { box-shadow: 0 0 0 0 rgba(232,48,58,.6); }
    50%      { box-shadow: 0 0 0 8px rgba(232,48,58,0); }
  }
  .nav-logo span { font-size: .8rem; letter-spacing: .15em; color: var(--muted); text-transform: uppercase; }
  ...
  .btn-primary:hover { background: #c42030; transform: translateY(-2px); box-shadow: 0 8px 25px rgba(232,48,58,.35); }
  .btn-outline {
  ...
  .stat-item .lbl { font-size: .78rem; color: var(--muted); letter-spacing: .08em; margin-top: .25rem; text-transform: uppercase; }
  .val-red    { color: var(--kritis); }
  .val-orange { color: var(--tinggi); }
  .val-blue   { color: var(--blue); }
  .val-green  { color: var(--rendah); }
  ...
  .section-sub { color: var(--muted); font-size: .9rem; margin-bottom: 3rem; }
  ...
  .method-card:hover { border-color: var(--blue); transform: translateY(-4px); }
  .method-tag {
  ...
  .tag-red   { background: rgba(239,68,68,.15); color: var(--kritis); }
  .tag-blue  { background: rgba(59,130,246,.15); color: var(--blue); }
  .method-card h3 { font-size: 1.4rem; margin-bottom: .8rem; }
  .method-card p  { color: var(--muted); font-size: .9rem; line-height: 1.6; }
  .method-steps {
  ...
  /* ── CRITERIA TABLE ── */
  .criteria-grid { display: grid; grid-template-columns: repeat(4,1fr); gap: 1rem; margin-top: 2rem; }
  .crit-card {
    background: var(--surface); border: 1px solid var(--border);
    border-radius: 10px; padding: 1.5rem;
  }
  .crit-card .icon { font-size: 1.8rem; margin-bottom: .75rem; }
  .crit-card h4 { font-size: .95rem; margin-bottom: .3rem; }
  .crit-card .type-badge {
    font-size: .7rem; padding: .2rem .6rem; border-radius: 4px;
    background: rgba(249,115,22,.15); color: var(--tinggi); font-weight: 600;
  }
  .crit-card p { font-size: .8rem; color: var(--muted); margin-top: .5rem; line-height: 1.5; }
  ...
  <div class="method-grid">
    ...
    <a href="dashboard.php?method=saw" class="method-card">
      ...
      <div class="method-steps">
        <div class="method-step"><div class="step-num">1</div>Bangun matriks perbandingan AHP 4×4</div>
        <div class="method-step"><div class="step-num">2</div>Hitung bobot kriteria & uji konsistensi</div>
        <div class="method-step"><div class="step-num">3</div>Normalisasi data pasien (cost/benefit)</div>
        <div class="method-step"><div class="step-num">4</div>Hitung skor SAW = Σ (Wj × Rij)</div>
      </div>
    </a>
    <!-- AHP+WP -->
    <a href="dashboard.php?method=wp" class="method-card">
      ...
      <p>Mengalikan nilai kriteria yang dipangkatkan dengan bobot AHP. Metode ini lebih sensitif terhadap perbedaan proporsional antar kriteria dan memberikan penalti lebih besar pada nilai ekstrem.</p>
      <div class="method-steps">
        <div class="method-step"><div class="step-num">1</div>Gunakan bobot AHP yang sama</div>
        <div class="method-step"><div class="step-num">2</div>Hitung S(i) = Π Xij^(±Wj)</div>
        <div class="method-step"><div class="step-num">3</div>Normalisasi: V(i) = S(i) / Σ S(k)</div>
        <div class="method-step"><div class="step-num">4</div>Ranking berdasarkan V(i) tertinggi</div>
      </div>
    </a>
  </div>
</div>
...
<div class="section" style="padding-top:0">
  ...
  <div class="section-sub">Semua menggunakan tipe <em>cost</em> - nilai lebih rendah = kondisi lebih kritis = prioritas lebih tinggi</div>
...
<footer>
  ICU Priority System &nbsp;·&nbsp; Dataset: WiDS Datathon 2020 &nbsp;·&nbsp; Metode: AHP + SAW / WP
</footer>
