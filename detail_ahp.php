  ...
  <style>
    ...
    * {
    ...
    @keyframes pulse {
      ...
      50% {
        box-shadow: 0 0 0 7px rgba(232, 48, 58, 0)
      }
    }
...
<body>
  <nav>
    <div class="nav-logo">
      <div class="dot"></div><span>ICU Priority</span>
    </div>
    <div class="nav-links">
  ...
  <div class="page">
    ...
    <div class="page-sub">Analytical Hierarchy Process — Matriks 4×4 &nbsp;|&nbsp; Kriteria:
      <?= implode(', ', $labels) ?>
    </div>
          ...
          <?php foreach ($d['criteria'] as $i => $c):
            ...
            <div style="margin-bottom: 1rem;">
              <div style="display:flex;justify-content:space-between;margin-bottom:.4rem;font-size:.85rem">
                ...
                <span style="color:var(--muted);font-size:.75rem"><?= round($percentage, 2) ?>%</span>
              </div>

              <div class="w-bar" style="display: flex; align-items: center; gap: 10px;">
                <div class="bar-track"
                  style="flex-grow: 1; background: #eee; height: 8px; border-radius: 4px; overflow: hidden;">
                  <div class="bar-fill"
                    style="width: <?= $percentage ?>%; background: #4A90E2; height: 100%; transition: width 0.3s ease;">
                  </div>
                </div>
                <span class="w-val"
                  style="font-family: monospace; font-size: 0.8rem; min-width: 45px; text-align: right;">
                  <?= number_format($w, 4) ?>
                </span>
              </div>
            </div>
      ...
      <div class="card">
        ...
        <div class="consist-grid">
          ...
          <div class="c-item">
            ...
            <div
              style="font-size:.75rem;margin-top:.3rem;color:<?= $d['is_consistent'] ? 'var(--green)' : 'var(--accent)' ?>">
    ...
    <div class="section">
      ...
      <div class="card">
        <div class="card-hd">
          <h3>Urutan Kepentingan Kriteria</h3>
        </div>
        <div class="tbl-wrap">
          <table>
            <thead>
            ...
            <tbody>
              ...
              // PERBAIKAN: Gunakan satu tag foreach yang bersih
              foreach ($sorted as $c => $w):
                ...
                <tr>
                  <td><?= $rank++ ?></td>
                  <td class="left">
                    <strong><?= $labels_map[$c] ?? $c ?></strong><br>
                    <span style="font-size:.72rem;color:var(--muted)"><?= $c ?></span>
                  </td>
                  <td><strong><?= number_format($w, 4) ?></strong></td>
                  <td>
                    <div class="bar-track"
                      style="width:80px; display:inline-block; background:#eee; height:8px; border-radius:4px; overflow:hidden; vertical-align:middle; margin-right:5px;">
                      <div class="bar-fill" style="width:<?= $percentage ?>%; background:#4A90E2; height:100%;"></div>
                    </div>
                    <span style="font-size:.85rem;"><?= round($percentage, 1) ?>%</span>
                  </td>
                  <td>
                    <span
                      style="font-size:.75rem; background:rgba(249,115,22,.1); color:var(--orange); padding:.2rem .6rem; border-radius:4px; font-weight:600;">
                      <?= isset($types[$c]) ? strtoupper($types[$c]) : '-' ?>
                    </span>
                  </td>
  ...
  <div style="text-align:center;margin:2rem 0">
    <a href="dashboard.php"
      style="display:inline-flex;align-items:center;gap:.5rem;padding:.85rem 2rem;background:var(--blue);color:#fff;border-radius:8px;text-decoration:none;font-size:.9rem;font-weight:600">
